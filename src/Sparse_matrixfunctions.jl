...
rowsum(x) = sum(x,2)
rowsum{T,P}(x::SubArray{T,2,P}) where {T,P<:SparseMatrixCSC} = (x.parent * sparse(x.indexes[2],ones(Int,length(x.indexes[2])), ones(Int,length(x.indexes[2])), size(x.parent,2),1))[x.indexes[1]]
colsum(x) = sum(x,1)
colsum{T,P}(x::SubArray{T,2,P}) where {T,P<:SparseMatrixCSC} = (sparse(ones(Int,length(x.indexes[1])), x.indexes[1], ones(Int,length(x.indexes[1])),1,size(x.parent,1))*x.parent)[x.indexes[2]]
...
function nzcols{T,P}(b::SubArray{T,2,P,Tuple{Vector{Int64},Vector{Int64}}} where {T,P<:SparseMatrixCSC}
  )
...
function nzcols{T,P}(b::SubArray{T,2,P,Tuple{UnitRange{Int64},UnitRange{Int64}}} where {T,P<:SparseMatrixCSC}
  )
...
function nzcols{T,P}(b::SubArray{T,2,P,Tuple{UnitRange{Int64},Vector{Int64}}} where {T,P<:SparseMatrixCSC}
  )
  return [k
    for (k,i) in enumerate(b.indexes[2])
    if b.parent.colptr[i]<b.parent.colptr[i+1] &&
...
function nzcols{T,P}(b::SubArray{T,2,P,Tuple{Vector{Int64},UnitRange{Int64}}} where {T,P<:SparseMatrixCSC}
  )
    brows = sort(unique(b.indexes[1]))
    return collect(i+1-start(b.indexes[2])
      for i in b.indexes[2]
...
function nzrows{T,P,U}(b::SubArray{T,2,P,Tuple{Vector{Int64}, U}} where {T,P<:SparseMatrixCSC, U<:Union{UnitRange{Int64}, Vector{Int}}}
  )
...
function nzrows{T,P,U}(b::SubArray{T,2,P,Tuple{UnitRange{Int64}, U}} where {T,P<:SparseMatrixCSC, U<:Union{UnitRange{Int64}, Vector{Int}}}
  )
    active = falses(length(b.indexes[1]))
    for c in b.indexes[2]
