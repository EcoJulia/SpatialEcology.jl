...
################################ IGNORE
#
# # almost 1000 times slower TODO `findin` is the culprit
# function nzrows2{T,P<:SparseMatrixCSC}(b::SubArray{T,2,P}
#   )
#     active = falses(length(b.indexes[1]))
#     for c in b.indexes[2]
...
# end
#
# function nzrows3{T,P<:SparseMatrixCSC}(b::SubArray{T,2,P}
#   )
#     brows = sort(unique(b.indexes[1]))
#     for c in b.indexes[2]
#         for r in nzrange(b.parent,c)
#             ind = findin_sort(b.parent.rowval[r], brows)
#             if ind > 0
#                 active[ind] = true
#             end
#         end
...
# end
#
# function nzrows4{T,P<:SparseMatrixCSC}(b::SubArray{T,2,P}
#   )
#     brows = sort(unique(b.indexes[1]))
#     active = falses(length(b.indexes[1]))
#     for c in b.indexes[2]
#       active[findin_sort2(b.parent.rowval[nzrange(b.parent,c)], brows)] = true
#     end
#     return find(active)
# end
#
# function findin_sort(x, sortedvec)
#     i = 1; n = length(sortedvec)
#     n > 0 || return 0
#     while i <= n && sortedvec[i] <= x
#         x == sortedvec[i] && return i
#         i += 1
#     end
#     0
# end
#
# function findin_sort2(vec1, sorted_vec)
#     ret = Vector{Int}(0)
#     v1 = sort(vec1)
#     i,j = start(v1), start(sorted_vec)
#     while i <= length(v1) && j <= length(sorted_vec)
#         if v1[i] == sorted_vec[j]
#             push!(ret, j)
#             i += 1
#         elseif v1[i] > sorted_vec[j] j += 1
#         else i += 1
#         end
#     end
#     ret
